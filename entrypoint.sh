#!/bin/bash
# entrypoint.sh - FIXED VERSION - Linux compatible
set -euo pipefail
...
try:
    ...
    print(f'⚠️ Local engine error: {e}')
    exit(1)
" 2>/dev/null; then
            ...
            log_warn "⚠️ Local engine test failed - using fallback"
        fi
    else
        log_warn "⚠️ engine.py not found - using fallback analysis"
    fi
...
# ============================================================================
# DATA FILES VERIFICATION - FIXED FOR LINUX
# ============================================================================
verify_data_files() {
    ...
    if [[ ! -d "$data_dir" ]]; then
        log_warn "⚠️ Data directory not found: $data_dir"
    ...
    if [[ $json_count -ge 3 ]]; then
        ...
        log_warn "⚠️ Limited data files - will use basic analysis"
    fi
...
try:
    ...
    try:
        ...
        print('⚠️ Local engine not available - will use fallback')
    except Exception as e:
        print(f'⚠️ Local engine issue: {e}')
...
" 2>/dev/null; then
        ...
        log_warn "⚠️ Some imports failed - continuing with fallbacks"
        return 0
    fi
}
...
# ============================================================================
setup_runtime() {
    log_step "⚙️ Setting up runtime environment..."
...
# ============================================================================
main() {
    log_step "🚀 Starting LexAI with Local Compliance Engine"
    echo "========================================================"
    ...
    # Start Streamlit - ENSURE CORRECT PATH
